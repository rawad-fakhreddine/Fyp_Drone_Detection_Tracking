#!/usr/bin/env python3
"""
ibvs_controller_node.py  —  v6.19  (adaptive speed feedforward)
================================================================
AI-Based Drone-to-Drone Detection and Tracking
Rawad Fakhredine | FYP Masters in Robotics | Supervisor: Ibrahim Sammour

v6.19 — alpha-rate feedforward:
  vx = K_far × sqrt(-err_a) × gain   +   Kd_a × (-d_err_a) × gain
       ↑ proportional (approach)          ↑ derivative (speed matching)

  When target escapes fast (d_err_a << 0), the feedforward automatically
  boosts vx to match. When target is stable or approaching, ff = 0.
  This replaces the brute-force K_far increase with adaptive speed matching.

  Ref: Chaumette & Hutchinson 2006 §4.3: "adaptive gain avoids the
  compromise between speed and stability of a fixed gain."

Changes from v6.17:
  K_far: 28 → 30              (slight increase, not aggressive)
  NEW: Kd_a = 100.0           (alpha-rate feedforward gain)
  NEW: ff_max = 1.5 m/s       (feedforward cap)
  max_vx: 2.0 → 3.0           (allows feedforward to reach full output)
  vel_smooth_normal: 0.5 → 0.35 (faster response to speed changes)
  Dead zone ±0.001 (kept from v6.17)
"""
...
class IBVSController:
    def __init__(self):
        ...
        self.prev_err_a=0.    # v6.19: NEW — for alpha derivative
        ...
        rospy.loginfo("[IBVS] v6.19 | K_far=%.0f Kd_a=%.0f ff_max=%.1f max_vx=%.1f dead=%.3f"
    ...
    def compute_velocities(self,gain_scale=1.):
        ...
        dea=(ea-self.prev_err_a)/self.dt   # v6.19: alpha derivative
        ...
        if self.in_recovery():
            ...
            # Proportional: approach based on alpha error
            vx_p = self.K_far * np.sqrt(-ea - self.DEAD_ZONE) * gain
            # Derivative feedforward: boost when alpha dropping (target escaping)
            ff = 0.0
            if dea < -0.0005:  # alpha is decreasing → target moving away
        ...
        while not rospy.is_shutdown():
            ...
            if not self.armed: self.phase="DISARMED"; pub=False
                ...
                if self.got_real_detection and self.alpha>self.alpha_min_valid:
                    ...
                    self.phase="APPROACH"
                    rospy.loginfo("[IBVS] Target acquired a=%.4f → APPROACH"%self.alpha)
