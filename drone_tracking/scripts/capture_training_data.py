#!/usr/bin/env python3
"""
capture_training_data.py — v3.0 (no trajectory tag)
=====================================================
AI-Based Drone-to-Drone Detection and Tracking
Rawad Fakhredine | FYP Masters in Robotics | Supervisor: Ibrahim Sammour

v3.0 simplification over v2.0:
  * No _traj parameter — just run: rosrun drone_tracking capture_training_data.py
  * No T1_/T2_ prefix in filenames — plain sequential: f000000.jpg
  * Safe indexing preserved (max_existing + 1, never overwrites)
  * All bbox/geometry/GT logic unchanged

OUTPUT STRUCTURE:
  ~/drone_detection/capture_v4/
    raw/
      images/   ← f000000.jpg, f000001.jpg, ...
      labels/   ← f000000.txt, f000001.txt, ...
    classes.txt ← always just "drone"

USAGE:
  # Run full pipeline first (both drones flying), then:
  rosrun drone_tracking capture_training_data.py

  # Optional: override max frames (default 800)
  rosrun drone_tracking capture_training_data.py _max_frames:=1200
"""
...
# ── Capture settings ──────────────────────────────────────────────────────────
SAVE_INTERVAL_S   = 0.15   # min seconds between saves (~7 fps)
MIN_BBOX_PX       = 6      # skip if projected box < 6px (too far/tiny)
MAX_BBOX_FRAC     = 0.90   # skip if box > 90% of image (too close)
...
def _safe_start_index(directory):
    """
    Returns max_existing_index + 1.
    Never overwrites existing frames even if there are numbering gaps.
    """
    ...
    indices = []
    for f in existing:
        stem = os.path.splitext(os.path.basename(f))[0]   # e.g. "f000042"
        digits = stem[1:]  # strip leading 'f'
        try:
...
class GTCapture:
    def __init__(self):
        ...
        # v3.0: max_frames is optional ROS param, default 800
        self.max_frames = int(rospy.get_param('~max_frames', 800))
        ...
        rospy.loginfo("[GTCapture] v3.0 | MAX=%d | Start index=%d"
    ...
    def _chaser_cb(self, m):
        ...
        self.chaser_pitch = (math.copysign(math.pi/2, sp) if abs(sp) >= 1
                             else math.asin(sp))
    ...
    def _phase_cb(self, m):
        self.ibvs_phase = m.data

    def _image_cb(self, msg):
        if self.saved_count >= self.max_frames:
            rospy.loginfo("[GTCapture] Done — %d/%d frames saved. Ctrl+C to exit."
        ...
        if not self._got_chaser or not self._got_target:
            return
        if self.ibvs_phase not in IBVS_SAVE_PHASES:
            return
        ...
        try:
            ...
            rospy.logwarn("[GTCapture] Image error: %s" % e)
            return

        # v3.0: plain sequential filename — no trajectory prefix
        ...
        if self.saved_count % 100 == 0 or self.saved_count == 1:
            ...
            rospy.loginfo(
                "[GTCapture] %d/%d | d=%.1fm area=%.0fpx² "
                "bbox=(%.3f,%.3f,%.3f,%.3f)"
                % (self.saved_count, self.max_frames,
                   d, area, cx_n, cy_n, w_n, h_n))
    ...
    def _compute_gt_bbox(self):
        ...
        u_c = FX*(cx__/cz__) + CX
        v_c = FY*(cy__/cz__) + CY
        if not (0 < u_c < IMG_W and 0 < v_c < IMG_H):
            return None
        ...
        if bw < MIN_BBOX_PX or bh < MIN_BBOX_PX:
            return None
        if bw/IMG_W > MAX_BBOX_FRAC or bh/IMG_H > MAX_BBOX_FRAC:
            return None
